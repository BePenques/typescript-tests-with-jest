...
describe('Cart - add', ()=>{
  ...
  it('should has two itens in cart', ()=>{
    const { sut } = makeSut()
    expect(sut.items.length).toBe(0)
    sut.addItem(makeProduct('prod1',10))
  ...
  it('should has two itens in cart, and add same product one', ()=>{
    const { sut } = makeSut()
  ...
  it('should add two itens and remove one',()=>{
    const { sut } = makeSut()
  ...
  it('should add two itens and remove two',()=>{
    const { sut } = makeSut()
    const product1 = makeProduct('prod1',10)
    sut.addItem(product1)
  ...
  it('should total equals 80, with two products many times',()=>{
    ...
    const prod2 = makeProduct('prod1',10);
    sut.addItem(prod1)
    sut.addItem(prod1)
    sut.addItem(prod1)
    sut.addItem(prod2)
